document.addEventListener('DOMContentLoaded', () => {
  ...
  cashInputs.forEach((item, index) => {
    item.addEventListener("input", () => {
      ...
      totalcash(item, index)
    });
  });


  const showvalue = (item, index) => {
    const money = [2000, 500, 200, 100, 50, 20, 10, 5, 2, 1]
    const count = item.value;
    cashTexts[index].innerHTML = count * money[index];
    totalcash;
  }
  ...
  const totalcash = () => {
    ...
    cashInputs.forEach((item, index) => {
      const money = [2000, 500, 200, 100, 50, 20, 10, 5, 2, 1]
      const count = item.value;
      total += count * money[index];
    });
    txtFinalCash.innerHTML = total;
    txtFinalCashInWords.innerHTML = convertToWords(total);
  }
